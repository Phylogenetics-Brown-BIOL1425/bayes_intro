...
of 338 (the two runs with prior = 0.05 have posteriors of 293 and 281). Parameter estimates are nearly identical under the different priors, as are marginal probability distributions and parameter traces. Overall, there is nothing striking about the results I gathered that suggests that the change from prior = 0.05 to prior = 0.10 had a significant impact on my results. The only possible significant difference is that posteriors were more extreme with a higher prior, but I don't know if the observed difference can truly be considered 'significant' in a statistical sense.

The trees generated by these runs do not differ prominently from those generated with a prior of 0.05 - the major clades all exist, with the unstable taxa *Tarsius syrichta* and *Daubentonia madagascariensis* in different locations. Like in the 0.05 GTR run, *Loris tardigradus* and *Nycticebus coucang* are positioned with (*Galago senegalensis* + *Perodicticus potto*) in one tree and not in the other. Overall, the trees generated by the different runs are quite similar, which makes sense in light of the parameters and marginal probability distributions. This may indicate that this dataset has a clear phylogenetic signal - the prior probability has little influence because the analysis will always reach a similar region of tree space.
